...
public class TiledLayer extends Layer {
	...
	public void setStaticTileSet(final Image image, final int n, final int n2) {
		...
		this.setWidth(this.columns * n);
		this.setHeight(this.rows * n2);
		final int n3;
	...
	public final void paint(final Graphics graphics) {
		...
		if (super.isVisible()) {
			...
			for (int anInt600 = super.getX(), i = 0; i < this.cells.length; ++i, anInt600 += this.cellHeight) {
				for (int anInt601 = super.getY(), length = this.cells[i].length, j = 0; j < length; ++j, anInt601 += this.cellWidth) {
					int animatedTile;
