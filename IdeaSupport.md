...
ситуации необходимо выполнить `kemulator -restore` либо нажать соответствующую кнопку в интерфейсе утилит.**

Неполный список когда `-restore` выполнить НУЖНО:

- git clone репозитория
- перенос проекта в другую папку (и/или на другую машину)
- перемещение кемулятора и/или его UEI
- перемещение прогарда
- любая правка в мидлетах (новый, удаление, переименование), как в манифесте так и в коде, как из эклипса так и из идеи.
...
## Многоманифестовость (работа с 2+ проектами)

В эклипсе "файлом проекта" является де-факто манифест будущего приложения, который хранится в корне под именем
Application Descriptor. Интеллиж же хочет чтоб MANIFEST.MF был назван как MANIFEST.MF. Помогают симлинки, которые, как
выяснилось, прекрасно переносятся гитом между линуксами и виндами.
И даже нормально распаковываются из зипа некоторыми архиваторами. Разумеется, **для корректной работы этого всего
"двойные" проекты необходимо держать на какой-то ФС с поддержкой симлинков, например НЕ на фат32.**
С учётом того что на календаре написан 2025 год, я не посчитал это критичной проблемой. Ntfs и ext4 вполне подойдут.
