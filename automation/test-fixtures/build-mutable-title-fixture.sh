...
SOURCE_FILE="$ROOT_DIR/automation/test-fixtures/src/fixtures/MutableTitleFixtureMidlet.java"
BUILD_DIR="$ROOT_DIR/automation/test-fixtures/build-mutable-title"
CLASSES_DIR="$BUILD_DIR/classes"
MANIFEST_FILE="$ROOT_DIR/automation/test-fixtures/mutable-title-fixture.mf"
DEFAULT_OUTPUT_JAR="$BUILD_DIR/mutable-title-fixture.jar"

usage() {
  cat <<'EOF'
Usage:
  ./automation/test-fixtures/build-mutable-title-fixture.sh [RUNTIME_CLASSPATH] [OUTPUT_JAR]

Builds the automation mutable-title fixture MIDlet JAR.

Arguments:
  RUNTIME_CLASSPATH  Optional KEmulator compile classpath entry. Auto-detected when omitted.
  OUTPUT_JAR         Optional destination jar path. Defaults to ./automation/test-fixtures/build-mutable-title/mutable-title-fixture.jar
EOF
}
