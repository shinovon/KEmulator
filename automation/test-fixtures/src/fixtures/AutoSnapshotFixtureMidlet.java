...
public final class AutoSnapshotFixtureMidlet extends MIDlet {
	...
	public AutoSnapshotFixtureMidlet() {
		menu = new List("Auto menu", List.IMPLICIT);
	...
	protected void startApp() {
		...
		Thread transition = new Thread(new Runnable() {
			public void run() {
				...
				Display.getDisplay(AutoSnapshotFixtureMidlet.this).callSerially(new Runnable() {
					public void run() {
						showEditor();
					}
				});
	...
	private void showEditor() {
		if (editor == null) {
			editor = new TextBox("Auto editor", "ready", 32, 0);
			editor.addCommand(backCommand);
			editor.setCommandListener(new CommandListener() {
				public void commandAction(Command command, Displayable displayable) {
					if (command == backCommand) {
						Display.getDisplay(AutoSnapshotFixtureMidlet.this).setCurrent(menu);
					}
				}
			});
		}
