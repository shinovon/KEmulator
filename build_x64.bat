@echo off
REM ============================================================
REM Build script for KEmulator nnx64 (x64 variant)
...
REM - SWT JAR: home\swt-win32-x86_64.jar
REM - Uses single-pass compilation to resolve circular deps
REM   between m3g_lwjgl, micro3d_gl, and KEmulator_base's 3d module.
REM ============================================================

REM If not already under watchdog, re-launch with 30s timeout
if "%1" neq "--watched" (
...
REM SWT JAR (Windows x64) — no quotes; the javac -cp argument is quoted as a whole
set CLASSPATH=%CLASSPATH%;%HOME_DIR%\swt-win32-x86_64.jar
...
if exist "%HOME_DIR%\KEmulator_x64.jar" del /f /q "%HOME_DIR%\KEmulator_x64.jar" >nul 2>&1

if exist "C:\Projects\KEADDON\KEADDON_x64\KEmulator.jar" del /f /q "C:\Projects\KEADDON\KEADDON_x64\KEmulator.jar" >nul 2>&1
...
echo.>> "%OUT_DIR%\all_sources.txt"
type "%PROJECT_DIR%\src_list_x64.txt" >> "%OUT_DIR%\all_sources.txt"
echo.>> "%OUT_DIR%\all_sources.txt"
...
%JAVAC% -source 8 -target 8 -cp "%CLASSPATH%" -d "%OUT_DIR%" -Xmaxerrs 2000 "@%OUT_DIR%\all_sources.txt" > "%TEMP%\javac_build_x64.log" 2>&1
set JAVAC_EXIT=%errorlevel%
type "%TEMP%\javac_build_x64.log" | findstr /i "error"
del "%TEMP%\javac_build_x64.log" 2>nul
...
cd "%FAT_DIR%"
%JAR% xf "%HOME_DIR%\swt-win32-x86_64.jar" >nul 2>&1
...
%JAR% cfm "%HOME_DIR%\KEmulator_x64.jar" "%CLEAN_DIR%\META-INF\MANIFEST.MF" -C "%CLEAN_DIR%" . 2>nul
if %errorlevel% neq 0 echo ERROR: JAR creation failed & exit /b %errorlevel%

REM Verify JAR is valid
%JAR% tf "%HOME_DIR%\KEmulator_x64.jar" >nul 2>&1
...
if exist "%HOME_DIR%\KEmulator_x64.jar" (
    copy /y "%HOME_DIR%\KEmulator_x64.jar" "C:\Projects\KEADDON\KEADDON_x64\KEmulator.jar" >nul 2>&1
) else (
...
set BUILD_STEP=done
echo %BUILD_STEP% > "%TEMP%\ke_build_step.txt"
echo Local:   %HOME_DIR%\KEmulator_x64.jar
echo Launcher: C:\Projects\KEADDON\KEADDON_x64\KEmulator.jar
echo To run: java -javaagent:"%%HOME_DIR%%\KEmulator_x64.jar" -Djava.library.path="%%HOME_DIR%%" -jar "%%HOME_DIR%%\KEmulator_x64.jar"
